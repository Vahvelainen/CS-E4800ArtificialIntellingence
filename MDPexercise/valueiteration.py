...
def value_of(mdp, s, a, v, gamma):
    ...
    S = mdp.successor_states(s, a)
    val = sum( mdp.P(s, a, s2) * ( mdp.R(s, a, s2) + gamma * v[s2] ) for s2 in S )
    return val
...
def value_iteration(mdp, gamma, epsilon = 0.001):
    ...
    for _ in range(max_iterations):
      ...
      new_V = {}
      for s in S:
         A = mdp.applicable_actions(s)
         val = max( value_of(mdp, s, a, V, gamma) for a in A )
