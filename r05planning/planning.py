...
def findPlan(source,transition,target,MAX : int):
    ...
    for T in range(0,MAX+1):
        ...
        #Create transition clauses
        transitions = []
        for i in range(0, T):
            transitions.append( instantiate(transition, i) )

        #Add source, transitions and target to same formula
        formula = AND([
            instantiate(source, 0),
            AND(transitions),
            instantiate(target, T)
        ])
        ## formula = ....
