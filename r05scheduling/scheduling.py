...
def at_most_one(formulas):
    ...
    return AND( map( lambda pair: NOT( AND(pair) ) , all_pairs(formulas)) )
...
def exactly_one_execution(tasks, time_interval):
    ...
    # for each task, create a clause of "exactly_one"( [variableT1, variableT2] )
    timeRange = range(time_interval[0] , time_interval[1])
    clauses = []
    for task in tasks:
        taskTimes = []
        for time in timeRange:
            taskTimes.append( variable(task, time) )
        clause = exactly_one( taskTimes )
...
def mutually_exclusive_tasks(tasks, resource_need, time_interval):
    ...
    timeRange = range(time_interval[0] , time_interval[1])
    clauses = []
    # bute force trough all pairs and see if resourses are the same, then add a clauses for each time
    for taskA, taskB in all_pairs(tasks):
        if resource_need[taskA] == resource_need[taskB]:
            for time in timeRange:
                #clause says not both of them at the given time
                clauses.append( NOT( AND([ variable(taskA, time), variable(taskB, time) ]) ) )
    return AND(clauses)
...
def task_ordering(orders, time_interval):
    ...
    clauses = []
    for former, latter in orders:

        for time in range(time_interval[0] , time_interval[1]):
           
           #Clauses for checking the latter is not in previous time slots or on the same
           latterClauses = []
           for beforeTime in range(time_interval[0], time + 1):
               latterClauses.append( NOT(variable(latter, beforeTime)) )

           clause = IMPL( variable(former, time), AND( latterClauses ) )
           clauses.append( clause )
