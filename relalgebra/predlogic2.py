...
class NOT:
    ...
    def fma2RA(self,dbSchema):
        # U(x) for x in freevars
        freeVarsFormula = reduce(AND,[ ATOM("U",[Var(x)]) for x in self.freeVars()])
        return RAdifference(freeVarsFormula.fma2RA(dbSchema), self.subformula.fma2RA(dbSchema) )
...
class FORALL:
    ...
    def fma2RA(self,dbschema):
        if self.var not in self.subformula.freeVars():
            ...
            # U(selfvar) divided by formula
            return RAdivision( self.subformula.fma2RA(dbschema), ATOM("U",[Var(self.var)]).fma2RA(dbschema) )
